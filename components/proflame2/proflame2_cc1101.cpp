...
// when the RX register set was added in the same commit.
static const uint8_t CC1101_CONFIG_TX[][2] = {
...
void ProFlame2Component::configure_cc1101() {
  for (size_t i = 0; i < sizeof(CC1101_CONFIG_TX) / sizeof(CC1101_CONFIG_TX[0]);
       i++) {
    this->write_register(CC1101_CONFIG_TX[i][0], CC1101_CONFIG_TX[i][1]);
  }
...
void ProFlame2Component::set_radio_mode_(RadioMode mode) {
  ...
  size_t entries = 0;
  if (mode == RadioMode::kTx) {
    table = CC1101_CONFIG_TX;
    entries = sizeof(CC1101_CONFIG_TX) / sizeof(CC1101_CONFIG_TX[0]);
  } else {  // kRx
    table = CC1101_CONFIG_RX;
    entries = sizeof(CC1101_CONFIG_RX) / sizeof(CC1101_CONFIG_RX[0]);
  }
